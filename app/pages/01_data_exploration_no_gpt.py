...
def create_visualization(df, query):
    if 'histogram' in query.lower():
        ...
        # Display column names in a text area
        st.text_area("List of all DataFrame columns:", columns, height=300)

        column = query.split('histogram of ')[1].split(' ')[0]
        ...
        columns = query.split('scatter plot of ')[1].split(' and ')
        ...
        column = query.split('bar chart of ')[1].split(' ')[0]
        if column in df.columns:
...
def app():
    ...
    query = st.text_input("Enter your visualization query (e.g., 'histogram of LateralLength_FT', 'scatter plot of LateralLength_FT and CumOil12Month', 'bar chart of Operator')")
    if query:
        create_visualization(st.session_state['df'], query)